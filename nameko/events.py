...
from nameko.standalone.events import get_event_exchange, get_event_bus_exchange
...
class EventDispatcher(Publisher):
    ...
    def setup(self):
        self.exchange = get_event_bus_exchange()
    ...
    def get_dependency(self, worker_ctx):
        ...
        def dispatch(event_type, event_data):
            ...
            event_data.update({"event_type": event_type})
            event_data.update({"source_service": self.container.service_name})

            routing_key = f"{self.container.service_name}-{event_type}"
...
class event_handler:
    def __init__(self, source_service, event_type):
        self.source_service = source_service
        self.event_type = event_type
        pass
         
    def __call__(self, f):
        # event_bus_register => {"properties_update": {"properties": ["handle_xxx", "handle_xxx2"]}}
        event_mp = event_bus_register.get(self.event_type)
        if event_mp:
            service_mp = event_mp.get(self.source_service)
            if service_mp:
                service_mp.append(f.__name__)
            else:
                event_mp[self.source_service] = [f.__name__]

        else:
            event_bus_register[self.event_type] = {self.source_service: [f.__name__]}

        def wrap(*args):
            f(*args)
        return wrap

class EventBus(Consumer):
    ...
    def setup(self):
        ...
        queue_name = "{}-event-bus".format(service_name)
        exchange = get_event_bus_exchange()
        ...
        route_keys = []
        for event_type in list(dict.keys(event_bus_register)):
            for source_service in list(dict.keys(event_bus_register[event_type])):
                route_keys.append(f"{source_service}-{event_type}")
        
        print('route_keys: ', route_keys)

        bindings = [binding(exchange=exchange, routing_key=route_key) for route_key in route_keys]

        self.queue = Queue(
            queue_name, exchange=exchange, bindings=bindings,
            durable=True, auto_delete=auto_delete, exclusive=exclusive)

        super(EventBus, self).setup()

event_bus = EventBus.decorator
